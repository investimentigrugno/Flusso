...
# SEZIONE 1: FORMATTAZIONE (TAB 1 e 2)
...
# SEZIONE 2: TRADINGVIEW SCREENER
...
# SEZIONE 3: YFINANCE E INDICATORI
...
def generate_technical_agent_analysis(signal: dict, ticker_symbol: str) -> str:
    prompt = f"Analista Tecnico. Analizza {ticker_symbol}: Segnale {signal['direction']} ({signal['confidence']*100:.0f}%), RSI {signal['rsi']:.1f}, ATR {signal['atr']:.2f} ({signal['atr_percent']:.1f}%), Volatilità {signal['volatility']:.1f}%, Risk/Reward 1:{signal['risk_reward_ratio']:.2f}, Entry {signal['entry_point']:.2f}, SL {signal['stop_loss']:.2f}, TP {signal['take_profit']:.2f}. Analisi BREVE (max 400 parole): 1) Interpretazione segnale, 2) Qualità volatilità, 3) Risk/Reward, 4) Raccomandazione, 5) RATING: Strong Buy/Buy/Hold/Sell."
    try:
...
def generate_fundamental_agent_analysis(ticker_data: dict, ticker_symbol: str) -> str:
    ...
    dcf_text = f"DCF: Intrinsic ${dcf['intrinsic_value']:.2f} vs Current ${dcf['current_price']:.2f} ({dcf['upside_downside']:.1f}%), WACC {dcf['wacc']*100:.1f}%, Rec: {dcf['recommendation']}" if dcf else "DCF non disponibile"
    prompt = f"Analista Fondamentale. Analizza {ticker_symbol}: P/E {info.get('trailingPE', 'N/A')}, ROE {info.get('returnOnEquity', 'N/A')}, Crescita {info.get('revenueGrowth', 'N/A')}, FCF {info.get('freeCashflow', 'N/A')}, Debt/Equity {info.get('debtToEquity', 'N/A')}. {dcf_text}. Analisi BREVE (max 400 parole): 1) Valutazione, 2) DCF vs Multipli, 3) Qualità FCF, 4) Solidità, 5) RATING: Strong Buy/Buy/Hold/Sell."
    try:
        response = call_groq_api(prompt, max_tokens=400)
        return response
    except Exception as e:
...
def generate_news_sentiment_agent_analysis(ticker_data: dict, ticker_symbol: str) -> str:
    ...
    prompt = f"Analista Sentiment. Notizie {ticker_symbol}: {news_text or 'Nessuna'}. Analisi BREVE (max 400 parole): 1) Sentiment (Pos/Neu/Neg), 2) Impatto prospettive, 3) Rischi, 4) Catalizzatori, 5) RATING: Strong Buy/Buy/Hold/Sell."
    try:
        response = call_groq_api(prompt, max_tokens=400)
        return response
    except Exception as e:
...
def generate_consensus_analysis(tech_analysis: str, fund_analysis: str, sentiment_analysis: str, ticker_symbol: str) -> str:
    prompt = f"Moderatore Team Analisti {ticker_symbol}. Tech: {tech_analysis[:100]}... Fund: {fund_analysis[:100]}... News: {sentiment_analysis[:100]}... Analisi BREVE (max 400 parole): 1) Consenso, 2) Accordi/Disaccordi, 3) Fattore critico, 4) Coerenza, 5) RACCOMANDAZIONE FINALE: Strong Buy/Buy/Hold/Sell, 6) Confidence, 7) Rischi."
    try:
        response = call_groq_api(prompt, max_tokens=400)
        return response
    except Exception as e:
...
def display_multi_agent_analysis(ticker_data: dict, signal: dict, ticker_symbol: str):
    ...
    col_info.info("⏳ Generazione analisi AI in corso...")
...
def stock_screener_app():
    ...
    with tab1:
        if not st.session_state.data.empty:
            ...
            st.dataframe(df[['Company', 'Symbol', 'Investment_Score', 'Price', 'Rating']].head(20), use_container_width=True)
        else: st.info("Clicca 'Aggiorna Dati' per iniziare")
    ...
    with tab2:
        if not st.session_state.top_5_stocks.empty:
            ...
            for idx, (_, stock) in enumerate(st.session_state.top_5_stocks.iterrows(), 1):
                st.markdown(f"**#{idx} {stock['Company']} ({stock['Symbol']})** - Score {stock['Investment_Score']:.1f} - ${stock['Price']}")
                st.caption(f"{stock['Country']} | {stock['Sector']} | {stock['Recommendation_Reason']}")
                st.markdown("---")
        else: st.info("Aggiorna dati per vedere i top picks")
