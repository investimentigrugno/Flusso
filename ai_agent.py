...
# ============================================================================
# CONFIGURAZIONE GROQ API (CLOUD - GRATUITO)
# ============================================================================
GROQ_API_URL = "https://api.groq.com/openai/v1/chat/completions"
MODEL_NAME = "llama-3.1-70b-versatile"  # Modello potente e veloce

def call_groq_api(prompt: str, max_tokens: int = 1000) -> str:
    """Chiama l'API Groq per generare risposte AI"""
    try:
        headers = {
...
def check_groq_connection() -> bool:
    """Verifica se la chiave API Groq è valida"""
    if not GROQ_API_KEY or GROQ_API_KEY == "TUA_CHIAVE_API_QUI":
        return False
    
    try:
        test_response = call_groq_api("test", max_tokens=10)
        return "Errore" not in test_response
    except:
        return False
...
1. **Punti di Forza** (3 bullet points max 150 caratteri ciascuno)
2. **Rischi Principali** (3 bullet points max 150 caratteri ciascuno)
3. **Probabilità di Successo**: Indica chiaramente "Probabilità: XX/100" dove XX è un numero da 0 a 100
4. **Sintesi** (1 frase max 200 caratteri)
...
def ai_agent_app():
    ...
    with col1:
        if check_groq_connection():
            ...
            st.info("Verifica la chiave API nel file ai_agent.py")
            api_ok = False
    
    with col2:
        st.info(f"🧠 Modello: Llama 3.1 70B")
        st.caption("⚡ Inferenza ultra-veloce su cloud Groq")
    ...
    # Pulsante analisi
    if not api_ok:
        st.warning("🔧 Verifica la configurazione API Groq")
        return
    ...
    if 'ai_top_3' in st.session_state:
        ...
        for idx, (col, company) in enumerate(zip([col_m1, col_m2, col_m3], top_3)):
            with col:
                delta_val = f"Score: {company['investment_score']:.1f}"
                st.metric(
